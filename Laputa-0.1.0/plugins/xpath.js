...
function fXPath_evaluate(oQuery, sExpression, fNSResolver, topPath) {
	...
	if (typeof oNode == "undefined")
		...
		var mExpression	= new cExpression(cString(sExpression), oStaticContext);

		oStaticContext.namespaceResolver	= null;

		var aSequence,
		oSequence	= new cQuery,
		mSequence	= new cQuery,
	...
	if (bOldW3 && oStaticContext == oHTMLStaticContext)
        ...
        var isPath = false;
        var depth = 0;
        var curLevel = oExpression['internalExpression']['items'][0];

        while (isPath == false || depth >= 5) {


            if (curLevel.constructor.name == 'cPathExpr')
            {
                mExpression['internalExpression']['items'][0] = curLevel;
                isPath = true;
                break;
            }
            else
        ...
        mSequence	= mExpression.evaluate(new cDynamicContext(oStaticContext, oNode, null, oAdapter));
	...
	for (var nIndex = 0, nLength = aSequence.length, oItem; nIndex < nLength; nIndex++)
        {
                  ...
                  mSequence.push(oAdapter.isNode(mItem = mSequence[nIndex]) ? mItem : cStaticContext.xs2js(mItem));
        }

        if (topPath == true)
        {
            return [oSequence,mSequence];
...
oObject.xpathTK	= function(sExpression, fNSResolver) {
